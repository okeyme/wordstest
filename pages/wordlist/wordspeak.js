...
/****===================================处理结果===============================** */
	...
	playCurrentWord(e) {
		...
		const audioContext = this.data.audioContext;
		audioContext.src = list[index].audio; // 设置音频源
		audioContext.play(); // 播放
  ...
  nextword:function(e){
		...
		const {recordedFilePath,currentIndex,list} = this.data;
    if (!recordedFilePath) {
      ...
      return;
		...
		if(this.data.currentIndex>=list.length-1){
			...
			let next_currentIndex = currentIndex+1;
			var currentWord= {
				text: list[next_currentIndex].word,
				phonogram: list[next_currentIndex].phonogram_letter,
				audio: list[next_currentIndex].audio,
			}		
	...
	intList: function(){
		...
		const shuffled = [...list].sort(() => Math.random() - 0.5); // 洗牌
		var shuffList = [];
		if (shuffled.length <= 10) shuffList = [...shuffled]; // 长度不足时返回全部
		else shuffList = shuffled.slice(0, 10); // 取前10个
		var currentWord= {
      text: shuffList[currentIndex].word,
			phonogram: shuffList[currentIndex].phonogram_letter,
			audio: shuffList[currentIndex].audio,
		}
		this.setData({list:shuffList, currentWord:currentWord});
	},
	//添加练习记录
	addWordExercise:function(){
		const { list,formattedTime} = this.data;
		var wordExecList = [], catearr = [];
		list.forEach((item,index)=>{
			let flag = 0;
			let category_id = item.category_id;
			let wc_id = item.wc_id;
