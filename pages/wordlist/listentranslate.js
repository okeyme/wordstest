...
Page({
  ...
  async initTraining(unitId) {
	...
	// 播放当前单词
	playCurrentWord(e) {
		const { shufflist } = this.data;
		const { index }  = e.currentTarget.dataset;
		const currentWord = shufflist[index];
	...
	tryAgain(){
		this.setData({showResult:false, formattedTime: "0分00秒",startTime: Date.now(),currentIndex:0,errorCount:0});
		this.intList();
	},
	formattedTime() {
	...
	selectOption:function(e){
		...
		const updatePaths = {};
		updatePaths[`shufflist[${currentIndex}].selected`] = translate;
		shufflist[currentIndex].selected = translate;
		this.setData({ ...updatePaths});
		if(translate==shufflist[currentIndex].translate){
		...
		setTimeout(() => {
			...
			if(this.data.currentIndex>=shufflist.length-1){
				...
				this.setData({optionList:optionList, currentIndex:next_currentIndex,errorCount:this.data.errorCount, formattedTime: formattedTime, fadeClass:'fadein-left'},()=>{
					this.autoPlay();
				});
			}
		}, 1500);
	...
	generateDefinitionOptions(correctWord) {
		...
		// 提取词义作为选项
		const distractorDefinitions = distractorWords.map(word => word.translate);
		const allOptions = [correctWord.translate, ...distractorDefinitions];
