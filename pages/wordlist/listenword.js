...
Page({
  ...
  async initTraining(unitId) {
	...
	selectOption:function(e){
		...
		const updatePaths = {};
		updatePaths[`shufflist[${currentIndex}].selected`] = word;
		shufflist[currentIndex].selected = word;
		this.setData({ ...updatePaths});
		if(word==shufflist[currentIndex].word){
	...
	generateDefinitionOptions(correctWord) {
		...
		// 提取词义作为选项
		const distractorDefinitions = distractorWords.map(word => word.word);
		const allOptions = [correctWord.word, ...distractorDefinitions];
