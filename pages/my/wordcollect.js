...
Page({
  ...
  onLoad(options) {
    ...
    // 监听收藏更新事件
    app.on('favoriteCollectionUpdated', this.onFavoriteCollectionUpdated);
  ...
  // 加载缓存数据
  loadCachedData() {
    const cacheKey = `wordCollection_${this.data.vgId}_favorite`;
    const localCache = wx.getStorageSync(cacheKey);
    
    if (localCache && localCache.words && localCache.words.length > 0) {
      console.log('使用收藏本缓存数据展示首屏');
      
      this.setData({
        allFavoriteWords: localCache.words,
  ...
  // 加载收藏本（支持分页）
  async loadFavoriteCollection(forceRefresh = false) {
    // 如果是刷新，重置分页状态
    if (forceRefresh) {
      this.setData({
        currentPage: 1,
    ...
    try {
      // 从wordDataManager获取完整收藏列表
      wordDataManager.getFavoriteCollection(vgId, (result) => {
        this.setData({ 
          isRefreshing: false,
          isLoadingMore: false 
        });
        ...
        if (!result.error && result.words) {
          ...
          console.log(`✅ 收藏本加载完成: 共 ${allWords.length} 个单词`);
        } else {
          app.showToast("加载失败");
        }
      });
  ...
  // 更新显示单词（根据当前页码）
  updateDisplayWords() {
    const { allFavoriteWords, currentPage, pageSize } = this.data;
    ...
    // 获取当前页的单词
    const currentPageWords = allFavoriteWords.slice(startIndex, endIndex);
  ...
  // 加载更多（上拉触底）
  loadMoreWords() {
    const { isLoadingMore, hasMore, currentPage, allFavoriteWords, pageSize } = this.data;
    ...
    const totalPages = Math.ceil(allFavoriteWords.length / pageSize);
    ...
    this.setData({
      ...
      // 检查是否还有更多
      const remaining = allFavoriteWords.length - (nextPage * pageSize);
      if (remaining <= 0) {
        this.setData({ hasMore: false });
      }
  ...
  // 收藏更新事件处理
  onFavoriteCollectionUpdated: function(data) {
    if (data && data.vgId == this.data.vgId) {
      console.log('收藏有更新，重新加载');
      
      // 清除缓存
      const cacheKey = `wordCollection_${data.vgId}_favorite`;
  ...
  // 播放单词音频
  wordAudio(e) {
    const { displayWords, audioContext, isPlaying } = this.data;
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          ...
          app.requestData('/word/removewordCollect', 'POST', data, (res) => {
            if (res && res.data.code === 0) {
              // 清除缓存并重新加载
              const cacheKey = `wordCollection_${vgId}_favorite`;
              wx.removeStorageSync(cacheKey);
  ...
  goThisWord(e) {
    ...
    // 在allFavoriteWords中找到该单词的实际索引
    const actualIndex = allFavoriteWords.findIndex(word => word.wc_id === displayWord.wc_id);
    
    // 构建idlist
    const idlist = allFavoriteWords.map(word => word.wc_id);
    
    let url = `/pages/wordlist/wordlearn?wc_id=${displayWord.wc_id}&idlist=${idlist.join(",")}&revise=1`;
    app.gotoPage(url);
  },

  onUnload() {
    // 移除监听
    app.off('favoriteCollectionUpdated', this.onFavoriteCollectionUpdated);
  ...
  onShareAppMessage() {
    return {
      title: '我的收藏本',
      path: '/pages/wordlist/wordcollect'
    };
  },
  ...
  // 在wordcollect.js中添加一个专门处理水平滑动的方法
  ...
  onDeleteClick(e) {
    ...
    wx.showModal({
      title: '确认删除',
      content: `确定要从收藏中移除单词 "${word.word}" 吗？`,
      confirmText: '移除',
      confirmColor: '#FF4444',
      cancelText: '取消',
      success: (res) => {
